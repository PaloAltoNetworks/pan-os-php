...
$site_array['panos']['CheckForSoftwareUpdate'] = array();
$site_array['panos']['CheckForSignatureUpdate'] = array();
$site_array['panos']['CheckForWildfireUpdate'] = array();
$site_array['panos']['CheckForVirusUpdate'] = array();

$site_array['panos']['CheckForSignatureUpdate']['featureName'] = 'contents';
$site_array['panos']['CheckForWildfireUpdate']['featureName'] = 'wildfire';
$site_array['panos']['CheckForVirusUpdate']['featureName'] = 'virus';
...
$site_array['panorama']['CheckForSoftwareUpdate'] = array();
$site_array['panorama']['CheckForSignatureUpdate'] = array();
$site_array['panorama']['CheckForWildfireUpdate'] = array();
$site_array['panorama']['CheckForVirusUpdate'] = array();

$site_array['panorama']['CheckForSignatureUpdate']['featureName'] = 'contents';
$site_array['panorama']['CheckForWildfireUpdate']['featureName'] = 'wildfire';
$site_array['panorama']['CheckForVirusUpdate']['featureName'] = 'virus';
...
function curl_request_upload($url, $fields = array(), $data = '' )
{
    global $debug;
    global $folder;
    ...
    if ( !empty( $fields ) )
    {
        ...
        $data = array();
        $data['file'] = new CURLFile($folder.'/'.$fields['file_name'],'multipart/form-data',$fields['file_name']);
...
function download_preparation( $versioninfo, $key1, $latest = false)
{
    global $folder;
    global $folder_latest;
    global $filter;
    global $debug;
    ...
    if( isset($versioninfo['ReleaseNotesFileName']) && file_exists( $folder."/".$versioninfo['ReleaseNotesFileName'] )  )
    {
        echo "      - file ".$folder."/".$versioninfo['ReleaseNotesFileName']." already exist.\n";
        $release_notes_available = true;
    }

    if( isset($versioninfo['ReleaseNotesFileName']) && file_exists( $folder . "/" . $versioninfo['FileName'] ) )
    {
        echo "      - file ".$folder . "/" . $versioninfo['FileName']." already exist.\n\n";
    }
    else
    {
        if(  $versioninfo['ReleaseNotesFileName'] != 'EMPTY')
        {
            ...
            {
                echo "      - download file ".$folder."/".$versioninfo['ReleaseNotesFileName']."\n";
                download_file($versioninfo['ReleaseNotesURL'], $folder . "/" . $versioninfo['ReleaseNotesFileName']);
            }
        }
        else
        ...
        if( !$filter['only_releaseNotes'] )
        {
            echo "      - download file ".$folder . "/" . $versioninfo['FileName']."\n\n";
            download_file($versioninfo['downloadURL'], $folder . "/" . $versioninfo['FileName']);
        }
        ...
        {
            if(  $versioninfo['ReleaseNotesFileName'] != 'EMPTY')
                copy_file($folder . "/" . $versioninfo['ReleaseNotesFileName'], $folder . "/".$folder_latest."/" . $key1 . ".html");
            if( !$filter['only_releaseNotes'] )
                copy_file($folder . "/" . $versioninfo['FileName'], $folder . "/".$folder_latest."/" . $key1);
        }
...
function get_software( $item, $search_product = false )
{
...
function install_software( $inputConnector, $versioninfo, $key1, $install, $device)
{
    /*
    ...
    */
    ...
    if( file_exists( $folder . "/" . $versioninfo['FileName'] ) )
    {
        echo "\n  - ".$key1."\n";
        echo "      - upload file ".$folder . "/" . $versioninfo['FileName']."\n\n";

        #$filecontent = file_get_contents($folder . "/" . $versioninfo['FileName']);
    }
    ...
    if( $key1 != 'CheckForSoftwareUpdate' )
    {
        if( !$file_available )
        {
            ...
            $response = curl_request_upload( $url, $post, $data );
...
function diff_dynamic_content( $key, $fields, $device, &$url_array )
{
    global $debug;
    global $protocol;
    global $url;
    ...
    if( $debug )
    {
        print "URL: ".$protocol.$url.$site_fix.$key."\n";
        print "POST fiels: ".$fields."\n";
    }

    $curl_response = curl_request( $protocol.$url.$site_fix.$key, $fields, $data, "BASIC");
    ...
    $file = fopen( $folder.'/'.$file_name, "w") or die("Unable to open file!");
    ...
    foreach ($x->childNodes AS $item)
    {
        ...
        if( $item->nodeName == 'AllSWVersionInfo' || $item->nodeName == 'AllSignatureVersionInfo' )
        {
            ...
            foreach( $item->childNodes AS $item2 )
            {
                ...
                {
                    $url_array[$device['serial']][$key][$item2->nodeName][$i] = get_software($item2, $search_product);
                    $i++;
                }
            }
        }
        ...
        if( $item->nodeName == 'LatestSoftwareVersionInfo' || $item->nodeName == 'LatestSignatureVersionInfo' )
            $url_array[$device['serial']][$key][$item->nodeName] = get_software( $item, $search_product );
    }
}
...
#create folder if not exist
if( !file_exists( $folder."/".$folder_latest) )
{
    if( !mkdir($concurrentDirectory = $folder . "/" . $folder_latest, 0777, TRUE) && !is_dir($concurrentDirectory) )
    {
...
foreach($device_array as $device)
{
    ...
    {
        foreach( $site_array['panos'] as $key => $site )
        {
            ...
            diff_dynamic_content( $key, $fields, $device, $url_array);

        }
    }
    else
    {
        foreach( $site_array['panorama'] as $key => $site )
        {
            $fields = 'serialnumber='.$device['serial'];
...
foreach( $url_array as $key_serial => $array )
{
    ...
    foreach( $array as $key1 => $different_sw )
    {
        ...
        foreach( $different_sw as $key2 => $versioninfo )
        {
            ...
            {
                if( $filter['All'] && $filter['SWVersionInfo'] )
                {
                    $allrelease = $versioninfo;
                    foreach( $allrelease as $versioninfo )
                    {
                        download_preparation( $versioninfo, $key1 );
                    }
            ...
            {
                //what about
                if( $filter['All'] && $filter['SignatureVersionInfo']  )
                {
                    $allrelease = $versioninfo;
                    foreach( $allrelease as $versioninfo )
                    {
                        if( $upload_counter == 0 )
                            download_preparation( $versioninfo, $key1, true );
                        #elseif(  $upload_counter < 2 )
                        else
                            download_preparation( $versioninfo, $key1 );

                        if ( $configInput['type'] == 'api' && $filter['upload']  && $upload_counter < 4 )
                        {
                            install_software($inputConnector, $versioninfo, $key1, $filter['install'], $device);
                            $upload_counter ++;
                        }
                    }
            ...
            {
                if( $filter['SWVersionInfo'] )
                {
                    download_preparation( $versioninfo, $key1, true );
                    //install software
                    if ( $configInput['type'] == 'api' && $filter['upload']  )
                    {
                        install_software($inputConnector, $versioninfo, $key1, $filter['install'], $device);
                    }
                }
            ...
            {
                if( $filter['SignatureVersionInfo'] )
                {
                    download_preparation($versioninfo, $key1, TRUE);
