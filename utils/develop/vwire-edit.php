...
//
// VirtualWire filter provided in CLI ?
//
...
/** @var VirtualWireCallContext[] $doActions */
$doActions = Array();
foreach( $explodedActions as &$exAction )
{
    ...
    if( !isset(VirtualWireCallContext::$supportedActions[$actionName]) )
    {
    ...
    $context = new VirtualWireCallContext(VirtualWireCallContext::$supportedActions[$actionName], $explodedAction[1]);
...
{
    $objectFilterRQuery = new RQuery('virtual-wire');
...
foreach( $objectsLocation as $location )
{
    ...
    if( $configType == 'panos')
    {
            ...
            $objectsToProcess[] = Array('store' => $pan->network->virtualWireStore, 'objects' => $pan->network->virtualWireStore->virtualWires());
            $locationFound = true;
        #}
