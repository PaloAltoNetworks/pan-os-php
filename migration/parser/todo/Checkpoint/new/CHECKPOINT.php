...
if ($action=="import"){
    ...
    //UPdate the relationship between rulebases and routing files
    $ids_to_load = array();
    $ruleBases = explode(';', $signatureid);
    foreach($ruleBases as $ruleBase){
        $tuple = explode('XPDT', $ruleBase);
        $routeName = $tuple[1];
        $id =$tuple[0];
        $ids_to_load[] = $id;
        $query = "UPDATE policies_to_import_Checkpoint SET routes=\"$routeName\", selected=1 WHERE id=$id ;";
        $projectdb->query($query);
    }
    $projectdb->query("DELETE FROM policies_to_import_Checkpoint WHERE processed IS NULL AND selected=0;");

    //Get rulebases that we need to load:
    $query ="SELECT ruleBase, routes, fileName FROM policies_to_import_Checkpoint WHERE selected = 1 AND processed IS NULL";
    $ruleBasesResult = $projectdb->query($query);
    $ruleBasesToProcess = array();
    while($data = $ruleBasesResult->fetch_assoc()){
        $ruleBaseName = $data['fileName'];
        $routesName = $data['routes'];
        $ruleBasesToProcess[$ruleBaseName] = ['ruleBase'=> $ruleBaseName, 'routes'=>$routesName];
    }

    $ids_to_load = implode(',', $ids_to_load);

//    print_r($ruleBasesToProcess);

    update_progress($project, '0.00', 'Reading config files',$jobid);

    # Check if the Policy file contains more than one rule-base
    $fileName=USERSPACE_PATH."/projects/$project/checkPointR77/PolicyName.W";
    ...
    if ($result!=-1){
        ...
        foreach($result as $policyName){
            if(isset($ruleBasesToProcess[$policyName]))
                getPolicyName($rulebase, str_replace(' ', '_', $policyName),$project);
            else{
                echo "The $policyName is not selected to be processed\n";
            }
        }
        $rulebase="";
        ...
        //$ruleBasesToProcess=[];
        $a["PolicyName.W"] = $ruleBasesToProcess["PolicyName.W"] ;
        $ruleBasesToProcess = $a;
    }
...
//    print_r($ruleBasesToProcess);
    ...
    $source="";
    $alreadyLoaded=false;
    foreach ($ruleBasesToProcess as $policyName => $ruleBaseTuple){

        //Get information regarding the ruleBase to load Check if we have a routes file we should assign to this rulebase
        $query = "SELECT id, ruleBase, routes, selected FROM policies_to_import_Checkpoint WHERE ruleBase = '$policyName' AND processed IS NULL LIMIT 1;";
        $ruleBaseResult = $projectdb->query($query);
        if($ruleBaseResult->num_rows>0){
            $data = $ruleBaseResult->fetch_assoc();

            if($data['selected']!= 1){  //This rulebase has not been selected to be processed. Skip
                break;
            }

            $routesFile = $data['routes'];
        }



        $rulebases=USERSPACE_PATH."/projects/$project/checkPointR77/rulebases_5_0.fws";
        if (file_exists($rulebases)){
            $migratecomments="--merge_AI=".USERSPACE_PATH."/projects/$project/checkPointR77/rulebases_5_0.fws";
        }else{
            $migratecomments="";
        }
        $policyName=str_replace(' ', '_', $policyName);
        $checkJsonParser=false;
        $fwdoc=USERSPACE_PATH."/projects/$project/checkPointR77/conf.fwdoc";
        $ParserPath=INC_ROOT."/bin/configurations/parsers/Checkpoint/checkpoint-parser.pl";
        if(!file_exists(USERSPACE_PATH."/projects/$project/checkPointR77/$policyName")){
            $policyFile = "PolicyName.W";
        }
        else{
            $policyFile = $policyName;
        }
        $Parse="/usr/bin/perl $ParserPath --rules=".USERSPACE_PATH."/projects/$project/checkPointR77/$policyFile --objects=".USERSPACE_PATH."/projects/$project/checkPointR77/objects_5_0.C $migratecomments> $fwdoc";
        shell_exec($Parse);
        ...
        if ( (file_exists($fwdoc)) and (filesize($fwdoc)!=0) ){
            ...
            if (!file_exists(USERSPACE_PATH."/projects/$project/checkPointR77/rules.txt")){
                $file = USERSPACE_PATH."/projects/$project/checkPointR77/conf.fwdoc";
                if (file_exists($file)) {
                    ...
                    $isInstall=false;
                    $isShared="0";
                    $Objects_File = USERSPACE_PATH.'/projects/' . $project . '/checkPointR77/objects.txt';
                    $Services_File = USERSPACE_PATH.'/projects/' . $project . '/checkPointR77/services.txt';
                    $Rules_File = USERSPACE_PATH.'/projects/' . $project . '/checkPointR77/rules.txt';
                    $Nat_File = USERSPACE_PATH.'/projects/' . $project . '/checkPointR77/natrules.txt';
                    $natrulesline="";
                    ...
                    foreach ($string as $line => $record) {
                        ...
                        if (preg_match("/^  \"objects\": \{/i", $record)) {
                            $Objects = "true"; continue;
                        }

                        if (preg_match("/^  \"natrules\": \[/i", $record)) {
                            $NatRules = "true"; continue;
                        }
                                    ...
                                    if ($enterObject = "true") {
                                        ...
                                        if (preg_match("/\}/", $record)) {
                                            ...
                                            if ($ObjectType == "cluster_member") {
                                                ...
                                                $objectline .= "$ObjectName;$ObjectType;;;${ObjectIP}${concatObjectIP6};;;$ObjectComment;$interfaces2;$isShared\n";
            ...
            if ($checkJsonParser==true){
                if (!file_exists(USERSPACE_PATH."/projects/$project/checkPointR77/objects.txt")){
                    $get_debug=file(USERSPACE_PATH.'/projects/'.$project.'/debug.txt');
                    foreach ($get_debug as $line => $debug_line){
                        if (preg_match('/KeyError/i',$debug_line)){
                            #list($kk,$errores)=split('\'',$debug_line);
            ...
            if ($getVsys->num_rows==0){
                ...
                if ($getnewVsys->num_rows==1){
                    ...
                    if ($getnewVsys->num_rows==1){
                        ...
                        $tmp_vsys0=explode("vsys",$tmp_vsys); $vsysID=intval($tmp_vsys0[1]); $vsysID++;
                        $vsys="vsys".$vsysID;
...
//            $getTemplate=$projectdb->query("SELECT id FROM templates_mapping WHERE filename='$filename';");
            ...
            //echo "SELECT id FROM templates_mapping WHERE filename='$template_name';";
            $getTemplate=$projectdb->query("SELECT id FROM templates_mapping WHERE name='$template_name';");
            ...
            get_routes($project,$source,$template,$vsys, $policyName);
    ...
    //echo "SELECT vsys FROM device_mapping WHERE filename='$filename';";
    if ($getAllvsys->num_rows>0){
        while($getAllvsysData=$getAllvsys->fetch_assoc()){
            $vsys=$getAllvsysData['vsys'];
            ##delete_unused($source,$vsys);
            //echo "Call Fix with $source and $vsys\n";
            //fix_destination_nat($source, $vsys, $skipDNAT);

            $query = "SELECT id FROM virtual_routers WHERE template='$template' AND source='$source' AND name = 'chkpt-vr-".$vsys."';";
            //echo "Getting virtual routers to do autozone:\n $query\n\n";
            ...
            if ($getVR->num_rows == 1) {
                ...
                if(!$skipDNAT) {
//                    echo "\n\nGoing to calculate autozone for DNATs for vsys $vsys and vr: $vr_id\n";
                    recalculate_Dst_basedOn_NAT($projectdb, $source, $vsys, $vr_id, $project, 'CheckpointR80');
//                    echo "Calculated autozone for DNATs for vsys $vsys\n";
                }
...
elseif($action == "get"){
    ...
    $path = USERSPACE_PATH."/projects/" . $project . "/checkPointR77/";

    $i = 0;
    $dirrule = opendir($path);

    $fwNames = array();
    if ($type == "existing_ruleBases") {
        while ($config_filename = readdir($dirrule)) {
            ...
            if (checkFiles2Import($config_filename)){
                ...
                if($filename == "PolicyName.W") {
                    $foundRulebases = get_RuleBasesNames($config_path, $config_filename); //Update policies_to_import defining existing policies in the configuration
                    if($foundRulebases==0){
                        storePolicyName($config_path, $config_filename);
                    }
                }
                if($filename == "rulebases_5_0.fws"){
                    $foundRulebases = get_RuleBasesNames($config_path, $config_filename);  //Update policies_to_import defining existing policies in the configuration
                    if($foundRulebases==0){
                        storePolicyName($config_path, $config_filename);
                    }
        ...
        $query = "SELECT * FROM policies_to_import_Checkpoint ORDER BY ruleBase ASC;";
        ...
        $response = [
            'total'         => $count,
            'ruleBasesNames'      => $myData,
        ];
        echo json_encode($response);
    }
    elseif($type == "existing_routes"){
        $routes = array();
        while ($config_filename = readdir($dirrule)) {
            $fileFullPath = $path.$config_filename;
            //Check that the file is not a ZIP/Rar
            ...
            if (checkFiles2Import($config_filename)){
                $config_path = $path . $config_filename;
                $filename = $config_filename;
                if(isRouteFile($config_path)){
                    $routes[] = ['name'=>$config_filename];
                }
            }
        }
 ...
 */
function get_RuleBasesNames(STRING $config_path, STRING $config_filename): int{
    global $projectdb;
    ...
    $rulebase=file($config_path);

    $result=getLineWithString($rulebase, ":rule-base");

    if (count($result)>0 && $result!=-1) {
        //Create the table policies_to_import if it doesn't exist
        $res = $projectdb->query("SHOW tables LIKE 'policies_to_import_Checkpoint'");
        if($res->num_rows==0){
            $query= "CREATE TABLE `policies_to_import_Checkpoint` ( ".
                "`id` INT NOT NULL AUTO_INCREMENT, ".
                "`routes` VARCHAR(255) NOT NULL, ".
                "`fileName` VARCHAR(255) NOT NULL, ".
                "`ruleBase` VARCHAR(255), ".
        ...
        foreach ($result as $ruleBase){
            $found++;
            //Check if this policy was already loaded before
            $query = "SELECT id FROM policies_to_import_Checkpoint WHERE ruleBase='$ruleBase' ;";
            $isDup = $projectdb->query($query);
            if ($isDup->num_rows == 0) {
                $query = "INSERT into policies_to_import_Checkpoint (ruleBase, fileName) VALUES ('$ruleBase','$ruleBase');";
                $projectdb->query($query);
            }
        }
    }
...
function isRouteFile(STRING $config_path){
//Get the first line
//    $line = trim(fgets(gzopen($config_path, 'r')));
//    if(preg_match('/^Destination\s*Gateway\s*Genmask\s*(.*)$/', $line, $output_array))
//        return true;

    //check for the first 10 lines
    $lineNumber=0;
    $handle = gzopen($config_path, 'r');
    while (($buffer = fgets($handle, 4096)) !== false) {
        if(preg_match('/^Destination\s*Gateway\s*Genmask\s*(.*)$/', trim($buffer), $output_array))
            return true;
        else
            $lineNumber++;
        if($lineNumber>10) {
...
function getLineStartsWithString($fileName, $str) {
    $lines = $fileName;
    $record=array();
    foreach ($lines as $lineNumber => $line) {
        $match = preg_match('/^\("##(.*)".*$/', $line, $output_array);
        if(isset($output_array[1])){
            $record[] = $output_array[1];
        }
    }
    if (count($record)==0){return -1;}
    else{
...
function storePolicyName(STRING $config_path, STRING $config_filename):int{
    global $projectdb;
    ...
    $result=getLineStartsWithString($rulebase, "##");
    ...
    if (count($result)>0 && $result!=-1) {
        ...
        foreach ($result as $ruleBase){
            $ruleBase=addslashes(normalizeNames($ruleBase));
            //Check if this policy was already loaded before
            $query = "SELECT id FROM policies_to_import_Checkpoint WHERE ruleBase='$ruleBase' ;";
            //echo $query."\n";
            $isDup = $projectdb->query($query);
            if ($isDup->num_rows == 0) {
                $query = "INSERT into policies_to_import_Checkpoint (ruleBase,fileName) VALUES ('$ruleBase', 'PolicyName.W');";
                $projectdb->query($query);
            }
        }
    }
...
function getPolicyName($fileName, $str,$project){
    ...
    $print=false;
    $tabs="";
    $policyFile=USERSPACE_PATH."/projects/".$project."/checkPointR77/".$str;
...
function clean_partial_files($project,$policyName){
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/rules.txt"))        {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/rules.txt");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/routes.txt"))       {unlink(USERSPACE_PATH."/projects/".$project."/routes.txt");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/routes.out"))       {unlink(USERSPACE_PATH."/projects/".$project."/routes.out");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/conf.fwdoc"))       {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/conf.fwdoc");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/natrules.txt"))     {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/natrules.txt");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/objects.txt"))      {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/objects.txt");}
    //if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/".$policyName))     {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/".$policyName);}
    //if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/".$policyName."_FWS")) {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/".$policyName."_FWS");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/services.txt"))     {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/services.txt");}
}

function clean_files($project){
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/rules.txt"))        {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/rules.txt");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/routes.txt"))       {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/routes.txt");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/routes.out"))       {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/routes.out");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/conf.fwdoc"))       {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/conf.fwdoc");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/natrules.txt"))     {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/natrules.txt");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/objects_5_0.C"))    {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/objects_5_0.C");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/objects.txt"))      {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/objects.txt");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/PolicyName.W"))     {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/PolicyName.W");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/PolicyName.W_FWS")) {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/PolicyName.W_FWS");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/rulebases_5_0.fws")){unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/rulebases_5_0.fws");}
    if (file_exists(USERSPACE_PATH."/projects/".$project."/checkPointR77/services.txt"))     {unlink(USERSPACE_PATH."/projects/".$project."/checkPointR77/services.txt");}
}

function clean_files_Dir($project) {
    $dirPath = USERSPACE_PATH."/projects/".$project."/checkPointR77";
    if (! is_dir($dirPath)) {
        throw new InvalidArgumentException("$dirPath does not exist");
    }
...
function get_objects($project,$source,$vsys){
    global $projectdb;
    $original_vsys=$vsys;
    $objectsfile=USERSPACE_PATH."/projects/$project/checkPointR77/objects.txt";
    ...
    foreach ($lines as $line_num => $line)
    {
        ...
        if ($ipversion=="v4"){$hostCidr="32";}
        if ($ipversion=="v6"){$hostCidr="128";}
        else {$ipversion="v4";}
        ...
        elseif($type == "gateway_fw"){
            $address[$ipversion][]="('ip-netmask','$type','$ip','$name','$name','$name','1','1','$description','$source','$vsys')";}
        ...
        elseif($type == "host"){
            if ($obj_interfaces==""){
                if(preg_match("/\+/i", $ipaddress)){
                    ...
                    $firstFound = false;
                    if($ip1 != ''){
                        //Put the data of IPv4here
                        $firstFound=true;
                        $ipversion=ip_version($ip1);
                        if ($ipversion=="v4"){$hostCidr="32";}
                        if ($ipversion=="v6"){$hostCidr="128";}
                        else {$ipversion="v4";$hostCidr="32";}
                    }
                    ...
                    if($firstFound && $secondFound){
                        $address[$ipversion][]="('ip-netmask','ip-netmask','$ip1','$hostCidr','${name}_exp${ipversion}','$name','0','1','$description','$source','$vsys')";
                        $addmembers[]="('$lid','${name}_exp${ipversion}','$source','$vsys')";
                        $address[$ipversion][]="('ip-netmask','ip-netmask','$ip2','$hostCidr','${name}_exp${ipversion}','$name','0','1','$description','$source','$vsys')";
                        $addmembers[]="('$lid','${name}_exp${ipversion}','$source','$vsys')";
                    }
                    else{
                        if($firstFound){
                            $address[$ipversion][]="('ip-netmask','ip-netmask','$ip1','$hostCidr','${name}','$name','0','1','$description','$source','$vsys')";
                            $addmembers[]="('$lid','${name}','$source','$vsys')";
                        }
                        if($secondFound){
                            $address[$ipversion][]="('ip-netmask','ip-netmask','$ip2','$hostCidr','${name}','$name','0','1','$description','$source','$vsys')";
                            $addmembers[]="('$lid','${name}','$source','$vsys')";
                        }
                    }
        ...
        elseif($type == "network"){
            if(preg_match("/\+/i", $ipaddress)){
                ...
                if($firstFound && $secondFound){
                    $address[$ipversion][]="('ip-netmask','ip-netmask','$ip1','$hostCidr1','${name}_exp${ipversion}','$name','0','1','$description','$source','$vsys')";
                    $addmembers[]="('$lid','${name}_exp${ipversion}','$source','$vsys')";
                    $address[$ipversion][]="('ip-netmask','ip-netmask','$ip2','$hostCidr2','${name}_exp${ipversion}','$name','0','1','$description','$source','$vsys')";
                    $addmembers[]="('$lid','${name}_exp${ipversion}','$source','$vsys')";
                }
                else{
                    if($firstFound){
                        $address[$ipversion][]="('ip-netmask','ip-netmask','$ip1','$hostCidr1','${name}','$name','0','1','$description','$source','$vsys')";
                        $addmembers[]="('$lid','${name}','$source','$vsys')";
                    }
                    if($secondFound){
                        $address[$ipversion][]="('ip-netmask','ip-netmask','$ip2','$hostCidr2','${name}','$name','0','1','$description','$source','$vsys')";
                        $addmembers[]="('$lid','${name}','$source','$vsys')";
                    }
                }
                ...
                /*if($name == "Net_16_and_over") {
                    $cidr_check = mask2cidrv4($mask);
                    if($cidr_check != "0"){
                        echo "---------------------------------------18. Name: " .$name. "\n";
                        echo "---------------------------------------18. CIDR: " .$mask. "\n";
                        echo "---------------------------------------18. CIDR22: " .$cidr_check. "\n";
                    }
                }*/
                ...
                if($cidr_check != "0"){
                    $warning_mask = $mask;
                    $mask = "32";
                    add_log('error','Phase 2: Reading Address Objects and Groups','Netmask out of the standard range: '.$warning_mask.' ', $source,'Added '.$name.' with ip '.$ip.' and cidr: '.$mask);
                }
    ...
    if ((isset($address['v6'])) AND(count($address['v6'])>0)){
        ...
        $projectdb->query("INSERT INTO address (type,vtype,ipaddress,cidr,name,name_ext,checkit,v6,description,source,vsys) VALUES ".implode(",",$unique).";");
    }
    unset($address);
...
function get_services($project,$source,$vsys){
    global $projectdb;
    $servicesfile=USERSPACE_PATH."/projects/$project/checkPointR77/services.txt";
    ...
    foreach ($lines as $line_num => $line){
        ...
        elseif($serviceprotocol == "group"){
            ...
            $servicegroups[]="('$lid','$name','$name','$source','0','$vsys','$description')";
            foreach ($servicemember as $servicemember_var){
                $servicesMembers[]="('$lid','$servicemember_var','$source','$vsys')";
            }
            $lid++;
        }
        elseif($serviceprotocol == "gtp"){$services[]="('$name','$name','$serviceprotocol','$serviceport','1','$description','$source','0','$vsys','$srcport')";}
        elseif($serviceprotocol == "gtp_mm_v0"){$services[]="('$name','$name','$serviceprotocol','$serviceport','1','$description','$source','0','$vsys','$srcport')";}
        elseif($serviceprotocol == "gtp_mm_v1"){$services[]="('$name','$name','$serviceprotocol','$serviceport','1','$description','$source','0','$vsys','$srcport')";}
        elseif($serviceprotocol == "gtp_v1"){$services[]="('$name','$name','$serviceprotocol','$serviceport','1','$description','$source','0','$vsys','$srcport')";}
        elseif($serviceprotocol == "icmp"){$services[]="('$name','$name','$serviceprotocol','$serviceport','1','$description','$source','1','$vsys','$srcport')";}
        elseif($serviceprotocol == "icmpv6"){$services[]="('$name','$name','$serviceprotocol','$serviceport','1','$description','$source','1','$vsys','$srcport')";}
        elseif($serviceprotocol == "other"){$services[]="('$name','$name','$serviceprotocol','$serviceport','1','$description','$source','0','$vsys','$srcport')";}
        elseif($serviceprotocol == "tcp_citrix"){$services[]="('$name','$name','tcp','$serviceport','1','$description','$source','0','$vsys','$srcport')";}
        elseif($serviceprotocol == "tcp_subservice"){$services[]="('$name','$name','$serviceprotocol','$serviceport','1','$description','$source','0','$vsys','$srcport')";}
        else{
            if ($serviceprotocol!=""){
                add_log('error','Phase 3: Reading Services Objects and Groups','Reading Contents',$source,'Unknown Service Protocol, please send a mail to fwmigrate@paloaltonetworks.com : '.$serviceprotocol);
            }
        }
    }
    ...
    if (count($servicegroups)>0){
        $projectdb->query("INSERT INTO services_groups_id (id,name,name_ext,source,checkit,vsys,description) VALUES ".implode(",",$servicegroups).";");
        unset($servicegroups);
...
function generate_viewer(){
    global $project;
    if (file_exists(USERSPACE_PATH."/projects/$project/checkPointR77/rulebases_5_0.fws")){
        $cmd="/usr/bin/perl /var/www/html/tools/cpoint_viewer/fw1rules.pl --output_html=".USERSPACE_PATH."/projects/$project/checkPointR77/CPviewer.html --rules=".USERSPACE_PATH."/projects/$project/checkPointR77/PolicyName.W --merge_AI=".USERSPACE_PATH."/projects/$project/checkPointR77/rulebases_5_0.fws  --objects=".USERSPACE_PATH."/projects/$project/checkPointR77/objects_5_0.C --with_ip --show_members --icon_path=/tools/cpoint_viewer/icons";
    }
    else{
        $cmd="/usr/bin/perl /var/www/html/tools/cpoint_viewer/fw1rules.pl --output_html=".USERSPACE_PATH."/projects/$project/checkPointR77/CPviewer.html --rules=".USERSPACE_PATH."/projects/$project/checkPointR77/PolicyName.W --objects=".USERSPACE_PATH."/projects/$project/checkPointR77/objects_5_0.C --with_ip --show_members --icon_path=/tools/cpoint_viewer/icons";
    }
    shell_exec($cmd);
}
...
function get_nat_policies($project,$source,$vsys,$convertObjectsShared){
    global $projectdb;
    $rulesfile=USERSPACE_PATH."/projects/$project/checkPointR77/natrules.txt";
    ...
    if ($getPosition->num_rows==0){$position=1;}
    else{
    ...
    foreach ($lines as $line_num => $line)
    {
        ...
        elseif ($src=="]"){
            ...
            if ($rule_enabled=="yes"){$rule_enabled="0";} else {$rule_enabled="1";}
            if ($firewall==""){$firewall="Any";}
...
function get_security_policies($project,$source,$vsys,$convertObjectsShared){
    ...
    if ($vsys==""){$vsys="vsys1";}
    $lid=0;
    $rulesfile=USERSPACE_PATH."/projects/$project/checkPointR77/rules.txt";
    $lines = file($rulesfile);
    $linesNumber = count(($lines));
    $headerB="";
    ...
    foreach ($lines as $line_num => $line){
        ...
        $checkit=0;
        if ($headerA==""){$header=$headerB;}else{$header=$headerA;$headerB=$headerA;}
        #Increment the lid
        #$lid++;
        ...
        elseif($action=="reject"){
            $action="drop";
            add_log2('warning','Reading Security Rules','Action is Reject in RuleID ['.$lid.'] and registered as DROP',$file_source,'Verify that changed Action to Drop is valid','rules',$lid,'security_rules');
        }
        else {
            $action="deny";
        }
        if ($rule_enabled=="yes"){$rule_enabled="0";} else {$rule_enabled="1";}
        if ($negate_source=="no"){$negate_source="0";}else{$negate_source="1";
        ...
        if ($negate_destination=="no"){$negate_destination="0";}else{$negate_destination="1";
...
function fix_destination_nat($source,$vsys, $skipDAT){
    ...
    if ($getVR->num_rows==1){
        ...
        // Calculate Zone FROM to NAT (only applied to the NAT rule itself)
        ...
        // Calculate Zone TO to NAT (only applied to the NAT rule itself)
...
function get_routes($project,$source,$template,$vsys,$policyName){
#Support for ipv4 by now
    global $projectdb;

    $query = "SELECT routes FROM policies_to_import_Checkpoint WHERE ruleBase = '$policyName' AND selected=1 LIMIT 1\n";
    $resultRoute = $projectdb->query($query);
    if($resultRoute->num_rows>0){
        $data = $resultRoute->fetch_assoc();
        $routes_file=USERSPACE_PATH."/projects/$project/checkPointR77/".$data['routes'];
    }
    else{
        $query = "SELECT routes FROM policies_to_import_Checkpoint WHERE fileName = 'PolicyName.W' AND selected=1 LIMIT 1\n";
        $resultRoute = $projectdb->query($query);
        if($resultRoute->num_rows>0){
            $data = $resultRoute->fetch_assoc();
            $routes_file=USERSPACE_PATH."/projects/$project/checkPointR77/".$data['routes'];
            ...
            $routes_file = USERSPACE_PATH . "/projects/$project/checkPointR77/routes.txt";
        }
    }

    $count=0;
    $routes_out =USERSPACE_PATH."/projects/$project/checkPointR77/routes.out";
    if (file_exists($routes_file)){
        ...
        #Create a VR
        $getVR=$projectdb->query("SELECT id FROM virtual_routers WHERE template='$template' AND name = 'chkpt-vr-".$vsys."';");
        if ($getVR->num_rows==0){
            $projectdb->query("INSERT INTO virtual_routers (name,source,template,vsys) VALUES ('chkpt-vr-$vsys','$source','$template','$vsys');");
        ...
        while ($dataLid = $getLid->fetch_assoc()) {
            ...
            foreach($incGroupExpanded_p as $index => $object){
                ...
                if($object['table_name'] == "address_groups_id"){
                    ...
                    $incGroupExpanded[$index] = Array(
                        'object'=>$object['member'],
                        'member_lid'=>$object['member_lid'],
                        'table_name'=>$object['table_name'],
                        'start'=>$res['start'],
                        'end'=>$res['end'],
                        'startip'=>long2ip($res['start']),
                        'endip'=>long2ip($res['end']), 'status'=>0);
                }
            }
...
function remove_duplicated_zones($source,$vsys){
    remove_duplicated_zones_from($source, $vsys);
    remove_duplicated_zones_to($source, $vsys);
}

function remove_duplicated_zones_to($source,$vsys){
    remove_duplicated_zones_direction($source, $vsys, "to");
}

function remove_duplicated_zones_from($source,$vsys){
    remove_duplicated_zones_direction($source,$vsys, "from");
}

function remove_duplicated_zones_direction($source,$vsys, $direction){
    global $projectdb;

    $query = "SELECT GROUP_CONCAT(id) as ids,count(id) AS t FROM security_rules_".$direction." WHERE source='$source' AND vsys='$vsys' GROUP BY name,rule_lid having t>1; ";
    $getDups=$projectdb->query($query);
    if ($getDups->num_rows>0){
        $remove=array();
        while($getDupsData=$getDups->fetch_assoc()){
            $deletable = explode(",", $getDupsData['ids']);
            array_shift($deletable);  //Select all the duplicated elements besides the first one
            $remove=array_merge($remove,$deletable);
        }
        $query = "DELETE FROM security_rules_".$direction." WHERE id IN (".implode(",",$remove).")";
        $projectdb->query($query);
    }
}