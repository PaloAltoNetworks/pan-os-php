	...
	while ( ($line = <INFILE> ) && ( fromdos("$line") ne "\t)" ) ) {
		...
		while ( ($line = <INFILE>) && ( fromdos("$line") ne "\t\t)" ) ) {
			...
			if ( $line =~ /^\t\t\t:location \(/ ){
				...
				if($netmasktranslation{$lineparam} == ""){
...
sub ReadNetworkObjects{
	...
	while ( ($line = <INFILE> ) && ( fromdos("$line") ne "\t)" ) ) {
		...
		while ( ($line = <INFILE>) && ( fromdos("$line") ne "\t\t)" ) ) {
			...
			elsif ( $line =~ /^\t\t\t:ipv6_prefix \(/ ){
				$obj_netmask6{$name} = "$lineparam";
			}
			elsif ( $line =~ /^\t\t\t:netmask6 \(/ ){
				#Calculate the CIDR based on the netmask for ipv6
				$obj_netmask6{$name} = $netmasktranslationV6{$lineparam};
			}
			elsif ( $line =~ /^\t\t\t:valid_ipaddr \(/ ){
				$obj_NATadr{$name} = $lineparam;
			}
...
#$obj_shared
		...
		if ( "$obj_ipaddr6{$name}" ne '' ) {
			$netmask = $obj_netmask6{$name};
			print "\t    ,\"ipaddr6\": ";
			if ($FLAG_anonymize) {
				print "\"A.N.O.N/YM\"\n";
			} else {
				if ( $netmask ne '' ) { $netmask = "/$netmask"; }
				print "\"$obj_ipaddr6{$name}$netmask\"\n";
			}
		}
