...
//Dependencies
require_once INC_ROOT.'/bin/configurations/parsers/ServicesMapping.php';
use PaloaltoNetworks\Parsers\Utils\ServicesMapping;
    ...
    while ($config_filename = readdir($dirrule)) {
        ...
        if (checkFiles2Import($config_filename)){
            ...
            if ($isUploaded->num_rows == 0) {
                ...
                #Remove DM_INLINE
                // check if exists some object with (name LIKE 'DM_INLINE_%' OR name LIKE 'CSM_INLINE_%' OR name LIKE 'FMC_INLINE_%')
                $dm_inline = false;
                $dm_inline_tables = ["address_groups_id", "services_groups_id"];
                foreach ($dm_inline_tables as $table_name) {
                    $query_inline = "SELECT name FROM $table_name where name LIKE 'DM_INLINE_%' OR name LIKE 'CSM_INLINE_%' OR name LIKE 'FMC_INLINE_%'";
                    $get_inline = $projectdb->query($query_inline);
                    if ($get_inline->num_rows > 0) {
                        $dm_inline = true;
                        break;
                    }
                }
                if($dm_inline) {
                    ...
                    if ($getRules->num_rows > 0) {
                        echo "Remove DM_INLINE: # lines: $getRules->num_rows \n";
                    ...
                    update_progress($project, '0.75', 'Replacing NAT DM_INLINE groups by Members, can take minutes', $jobid);
                    #From Nat rules
                    $getRules = $projectdb->query("SELECT id FROM nat_rules WHERE source='$getSourceVsys[0]' AND vsys='$getSourceVsys[1]';");
                    if ($getRules->num_rows > 0) {
                        echo "Remove NAT DM_INLINE: # lines: $getRules->num_rows \n";
                        while ($getRulesData = $getRules->fetch_assoc()) {
                            $rule_lid = $getRulesData['id'];
                    ...
                    echo "No DM_INLINE objects created \n";
                }
                ...
                update_progress($project, '0.85', 'Recalculating Sec Rules destination based on NAT rules, can take minutes',$jobid);
...
function clean_config($config_path, $project, $config_filename) {
    ...
    $cisco_config_file = str_ireplace("\x0D", PHP_EOL, $cisco_config_file);
    $data = array();
...
//    update_progress($project, '0.90', 'Importing IPSEC VPNs');
    ...
    $version = getVersion($panos_version);
    ...
    foreach ($cisco_config_file as $line => $names_line) {
            ...
            foreach ($split as $splitelement) {
                switch ($splitelement) {
                    ...
                    case "aes-256":
                        ...
                        break;
                    case "aes-gcm-256":
                        $cryptoProfiles["$ikev2ProposalName"]->addEncryption("aes-gcm-256");
                        break;
                }
            }
        }
            ...
            foreach ($split as $splitelement) {
                switch ($splitelement) {
                    ...
                    case "sha-512":
                        ...
                        break;
                    case "null":
                        $cryptoProfiles["$ikev2ProposalName"]->addAuthentication("none");
                        break;
                }
            }
        }
        ...
        if ((preg_match("/crypto ipsec transform-set/i", $names_line)) OR ( preg_match("/crypto ipsec ikev1 transform-set/i", $names_line))) {
            ...
            switch ($split[4]) {
                ...
                case "mode":
                    break;
                case "esp-aes256gcm":
                    $cryptoProfiles["$split[3]"]->addEncryption("aes-gcm-256");
                    break;
                default:
        ...
        if (preg_match("/crypto map /", $names_line)) {
            ...
            elseif(isset($ipsecTunnels["$cryptoMapName-$cryptoMapPriority"])){
                ...
                elseif (($split[4] == "set") AND ( $split[5] == "pfs")) {
                    ...
                    if (($version < 10  && $pfs != "group21" && $pfs != "group24") ||
                        ($version >= 10 && $pfs != "group24")) {
                        if (isset($transformSet)) {
                            $transformSet->addDHgroup($pfs);
                        }
                    } else {
                        add_log2('error', 'Reading IKE Crypto Profiles', 'The PFS group ' . $pfs . ' is not supported yet.', $source, 'Group20 has been assigned instead on Profile[' . $cryptoMapName ."-".$cryptoMapPriority . ']', '', '', '');
                        $pfs = "group20";
                        if (isset($transformSet)) {
                            $transformSet->addDHgroup($pfs);
                        }
        ...
        if (($isIkev1Profile === TRUE) AND ( preg_match("/authentication /", $names_line))) {
            ...
            if (preg_match("/pre-share/", $names_line)) {
                ...
                $isIkev1Profile = FALSE;
                add_log2('error', 'Reading IKE Crypto Profiles', 'Only Pre-Shared key Profiles are supported', $source, 'Ignoring ' . $IkeCryptoProfileID . "-". $ikeCryptoProfileName . "[" . $names_line ."]", '', '', '');
            }
        }
        ...
        if (($isIkev1Profile === TRUE) AND ( preg_match("/encryption /", $names_line))) {
            ...
            switch ($split[1]) {
                ...
                case "aes-256":
                    ...
                    break;
                case "aes-gcm-256":
                    $IkeCryptoProfiles[$ikeCryptoProfileName]->addEncryption("aes-gcm-256");
                    break;
                default:
...
//                    echo "$groupName has timeout $timeout\n";
                ...
                $newRule->setName(substr($groupName, 0, (int)(63-6)) . "_" . $newRule->position);
...
//                                }
                            ...
                            case "host":
                                ...
                                elseif(in_array('destination', $missingFields)) {
                                    ...
                                    echo "Problem 2413: Host value in a wrong position? $names_line\n";
                                }
...
//                                        print_r($object);
                                ...
                                elseif ($versionCheck == "v6") {
                                    ...
                                    elseif(in_array('destination', $missingFields)) {
                                        ...
                                        echo "Problem 2613: Field $currentField not recognized in [$names_line]\n";
                                    }
                                }
...
function get_service_by_protocol($service_name, $protocol, $source, $vsys){
    global $projectdb;
    $service_item = [];
    // 1) Check if a service_group with defined $service_name exists
    $services_group = $projectdb->query("SELECT id FROM services_groups_id WHERE source='$source' AND BINARY name_ext='$service_name' AND vsys='$vsys' LIMIT 1");
    if ($services_group->num_rows > 0) {
        $service_group_item = $services_group->fetch_assoc();
        $service_group_id = $service_group_item['id'];
        // 1.1) Go over all the group members - only services - and search for one with similar $service_name and $protocol
        $query = "SELECT s.id, s.name, s.protocol, s.dport FROM services_groups sg, services s where sg.member_lid = s.id and sg.lid = '$service_group_id' and sg.table_name = 'services' and upper(s.name) like upper('%$service_name%') and upper(protocol) = upper('$protocol') LIMIT 1";
        $services = $projectdb->query($query);
        if ($services->num_rows > 0) {
            $service_item = $services->fetch_assoc();
        }
    }else{
        // 2) Check if there is a service with the defined $service_name and $protocol (none or only 1 shall exist)
        $services = $projectdb->query("SELECT name, id, protocol, dport FROM services WHERE source='$source' AND BINARY name_ext='$service_name' AND vsys='$vsys' and protocol = '$protocol' LIMIT 1");
        if ($services->num_rows > 0) {
            $service_item =  $services->fetch_assoc();
        } else {
            $services = $projectdb->query("SELECT name, id, protocol, dport FROM services WHERE source='$source' AND BINARY name_ext in ('".$protocol."_".$service_name."','".$protocol."-".$service_name."','".$service_name."_".$protocol."', '".$service_name."-".$protocol."') AND vsys='$vsys' and protocol = '$protocol' LIMIT 1");
            if ($services->num_rows > 0) {
                $service_item =  $services->fetch_assoc();
            }
        }
            ...
            if (preg_match("/port-object /", $names_line)) {
                ...
                if ($operator == "eq") {
                    ...
                    if (is_numeric($port)) {
                        ...
                        // try to search the service that is mapped using $service_name in the db (see .csv files on content/parsers folder)
                        $port_final = "";
                        $cisco_service = get_service_by_protocol($service_name, $Protocol, $source, $vsys);
                        if(isset($cisco_service['dport'])){
                            $port_final = $cisco_service['dport'];
                        }


                        if ($port_final == "" && $Protocol != "tcp-udp") {
                    ...
                    if ($port_final == "mt-error") {
                        ...
                        if ($Protocol == "tcp-udp") {
                            # TCP AND UDP
                            $cisco_tcp_service = get_service_by_protocol($service_name, "tcp", $source, $vsys);
                            if(isset($cisco_tcp_service['id'])){
                                $serviceID = $cisco_tcp_service['id'];

                            } else {
                                $projectdb->query("INSERT INTO services (name,name_ext,protocol,dport,checkit,source,vsys) values ('".$port_final."-tcp','".$port_final."-tcp','tcp','$port_final','0','$source','$vsys');");
                                $serviceID = $projectdb->insert_id;
                            }
                            $addMemberID[] = "('$lidgroup','services',$serviceID,'$source','$vsys')";

                            $cisco_udp_service = get_service_by_protocol($service_name, "udp", $source, $vsys);
                            if(isset($cisco_udp_service['id'])){
                                $serviceID = $cisco_udp_service['id'];
                            } else {
                                $projectdb->query("INSERT INTO services (name,name_ext,protocol,dport,checkit,source,vsys) values ('".$port_final."-udp','".$port_final."-udp','udp','$port_final','0','$source','$vsys');");
                                $serviceID = $projectdb->insert_id;
                            }
                            ...
                            // before creating a new service let's check if the service is already created
                            $cisco_service = get_service_by_protocol($service_name, $Protocol, $source, $vsys);
                            if(isset($cisco_service['id'])){
                                $serviceID = $cisco_service['id'];
                            }else{
                                //$srv_to_search = $Protocol."-".$port_final;
                                $srv_to_search = $port_final ."-". $Protocol;

                                # TCP OR UDP
                                $search = $projectdb->query("SELECT id FROM services WHERE source='$source' AND BINARY name_ext='$srv_to_search' AND vsys='$vsys';");
                                if ($search->num_rows == 0) {
                                    $projectdb->query("INSERT INTO services (name,name_ext,protocol,dport,checkit,source,vsys) values ('$srv_to_search','$srv_to_search','$Protocol','$port_final','0','$source','$vsys');");
                                    $serviceID = $projectdb->insert_id;
                                }
                                else{
                                    $data = $search->fetch_assoc();
                ...
                elseif ($operator == "range") {
                    ...
                    if (is_numeric($port_first)) {
                        ...
                        $port_first_port = "";
                        if($Protocol === "tcp-udp"){
                            $cisco_service = get_service_by_protocol($port_first, "tcp", $source, $vsys);
                            if(!isset($cisco_service['dport'])){
                                $cisco_service = get_service_by_protocol($port_first, "udp", $source, $vsys);
                                if(isset($cisco_service['dport'])){
                                    $port_first_port = $cisco_service['dport'];
                                }
                            }else{
                                $port_first_port = $cisco_service['dport'];
                            }
                            ...
                            $cisco_service = get_service_by_protocol($port_first, $Protocol, $source, $vsys);
                            if (isset($cisco_service['dport'])) {
                                $port_first_port = $cisco_service['dport'];
                            }
                    ...
                    if (is_numeric($port_last)) {
                        ...
                        if($Protocol === "tcp-udp"){
                            $cisco_service = get_service_by_protocol($port_last, "tcp", $source, $vsys);
                            if(!isset($cisco_service['dport'])){
                                $cisco_service = get_service_by_protocol($port_last, "udp", $source, $vsys);
                                if(isset($cisco_service['dport'])) {
                                    $port_last_port = $cisco_service['dport'];
                                }
                            } else {
                                $port_last_port = $cisco_service['dport'];
                            }
                            ...
                            $cisco_service = get_service_by_protocol($port_last, $Protocol, $source, $vsys);
                            if(isset($cisco_service['dport'])){
                                $port_last_port = $cisco_service['dport'];
                            }
                        ...
                        if ($port == "destination") {
                            ...
                            if ($port == "eq") {
                                ...
                                if (is_numeric($port)) {
                                    ...
                                    if ($getService->num_rows == 0) {
                                        # Create it
                                        $projectdb->query("INSERT INTO services (name,name_ext,protocol,dport,checkit,source,vsys) values ('$port_final-$Protocol','$port_final-$Protocol','$Protocol','$port_final','0','$source','$vsys');");
                                        $serviceID = $projectdb->insert_id;
                                        $addMemberID[] = "('$lidgroup','services',$serviceID,'$source','$vsys')";
                                    } else {
                                        $data = $getService->fetch_assoc();
                                    ...
                                    $cisco_service = get_service_by_protocol($port, $Protocol, $source, $vsys);
                                    if(isset($cisco_service['id'])){
                                        $serviceID = $cisco_service['id'];
                                        $addMemberID[] = "('$lidgroup','services',$serviceID,'$source','$vsys')";
...
//                                        $addMember[] = "('$lidgroup','$port','$source','$vsys')";
                ...
                elseif ($Protocol == "tcp-udp") {
                    ...
                    if ($port == "eq") {
                        ...
                        $cisco_tcp_service = get_service_by_protocol($port, "tcp", $source, $vsys);
                        if(isset($cisco_tcp_service['id'])){
                            $serviceID = $cisco_tcp_service['id'];
                        } else {
                            if(is_numeric($port)){
                                ...
                                add_log('error', 'Reading Services Objects and Groups', 'Unknown TCP Service [' . $HostGroupName . '] port-name mapping for: ' . $port, $source, 'Using 6500 port. Change it from the GUI');
                                $port_final = "6500";
                            }
                        ...
                        $cisco_udp_service = get_service_by_protocol($port, "udp", $source, $vsys);
                        if(isset($cisco_udp_service['id'])){
                            $serviceID = $cisco_udp_service['id'];
                        } else {
                            if(is_numeric($port)){
                                ...
                                add_log('error', 'Reading Services Objects and Groups', 'Unknown UDP Service [' . $HostGroupName . '] port-name mapping for: ' . $port, $source, 'Using 6500 port. Change it from the GUI');
                                $port_final = "6500";
                            }
                            $projectdb->query("INSERT INTO services (name,name_ext,protocol,dport,checkit,source,vsys) values ('udp-$port_final','udp-$port_final','udp','$port_final','1','$source','$vsys');");
                            $serviceID = $projectdb->insert_id;
                        }
                        $addMemberID[] = "('$lidgroup','services',$serviceID,'$source','$vsys')";
...
function add_cisco_services($source, $vsys) {
    ...
    if ($exists->num_rows == 0) {
        ...
        // Add predefined services and services groups
        ServicesMapping::addPredefinedSrvAndSrvGroups($projectdb, "cisco", $source, $vsys);
...
function get_object_service($cisco_config_file, $source, $vsys) {
    ...
    $isObjectService = 0;
    $addService = array();
    $invalid_service_list = array(); // list to store invalid services that requires to be flagged as invalid
    ...
    foreach ($cisco_config_file as $line => $names_line) {
        ...
        if ($isObjectService == 1) {
            ...
            if (( preg_match("/^network-object/i",    $names_line)) ||
                ( preg_match("/^access-list/i",    $names_line)) ||
                ( preg_match("/^nat/i",   $names_line)) ||
                ( preg_match("/^object network/i", $names_line)) ||
                ( preg_match("/^access-group/i", $names_line)) ||
                ( preg_match("/^route/i", $names_line))) {

                $isObjectService = 0;
                continue;
            }elseif (preg_match("/^service protocol/i", $names_line)) {
                $netObj = preg_split('/\s/', $names_line, 0, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
                ...
                if ($search->num_rows == 0) {
                    $addService[] = "('$ObjectServiceNamePan','$ObjectServiceName','$srv_protocol','','65535','','','1','$source','$vsys')";
                    ...
                    $invalid_service_list[]= "('$ObjectServiceNamePan')";
                }
            }
            ...
            elseif ((preg_match("/^service tcp/i", $names_line)) or ( preg_match("/^service udp/i", $names_line))) {
                ...
                $add_to_invalid_service_list = false;
                if(isset($netObj[2])){
                    if (($netObj[2] == "source") or ( $netObj[2] == "destination")) {
                        if ($netObj[3] == "eq") {
                            ...
                            if ($port_final == "") {
                                add_log('error', 'Reading Services Objects and Groups', 'Unknown Service [' . $ObjectServiceName . '] port-name mapping for: ' . $port, $source, 'Using 65535 port. Change it from the GUI');
                                $port_final = "65535";
                                $add_to_invalid_service_list = true;
                            }
                        }
                        elseif ($netObj[3] == "neq") {
                            $port = $netObj[4];
                        ...
                        elseif ($netObj[3] == "range") {
                            ...
                            if (is_numeric($port_first)) {
                                ...
                                if ($searchname->num_rows==1){
                                    ...
                                    if ($port_first_port == "") {
                                        add_log('error', 'Reading Services Objects and Groups', 'Unknown Service-Range [' . $ObjectServiceName . '] port-name mapping for: ' . $port_first, $source, 'Using 65535 port. Change it from the GUI');
                                        $port_first_port = "65535";
                                        $add_to_invalid_service_list = true;
                                    }
                                }
                            }
                ...
                if ($search->num_rows == 0) {
                    ...
                    if(isset($add_to_invalid_service_list) && $add_to_invalid_service_list === true){
                        $invalid_service_list[]="('$ObjectServiceNamePan')";
                    }
                }
                unset($add_to_invalid_service_list);
            }
            elseif (preg_match("/^service /i", $names_line)){
                // syntax example:
                // object service myServiceGRE
                //      service gre
                //      description myServiceGRE
                if (!in_array($ObjectServiceNamePan, $invalid_service_list, true )) {
                    // if service is not previously created
                    $addService[] = "('$ObjectServiceNamePan','$ObjectServiceName','tcp','','65535','','','1','$source','$vsys')";
                    add_log('warning', 'Reading Services Objects and Groups', 'Service Protocol found [' . $ObjectServiceName . ']', $source, 'Replace it by the right app-id');
                    $invalid_service_list[] = "('$ObjectServiceNamePan')";
                }
            }
        }
...
function compare_nat_address($original_address, $translated_address, $source, $vsys){
    global $projectdb;
    $original_address_member = [];
    $translated_address_member = [];
    if($original_address === $translated_address) {
        return true;
    } else {
        // look if original_address is a single_address or a group_address
        $getAddress = $projectdb->query("SELECT ipaddress,cidr,id,type FROM address WHERE BINARY name_ext='$original_address' AND source='$source' AND vsys='$vsys' AND vtype='object';");
        if ($getAddress->num_rows == 1) {
            // $original_address is a single address different that the $translated_address
            return false;
        } else {
            // get the members for the given $original_address
            $getAddressGroup = $projectdb->query("SELECT id FROM address_groups_id WHERE BINARY name_ext='$original_address' AND source='$source' AND vsys='$vsys';");
            if ($getAddressGroup->num_rows == 1) {
                $getAddressGroupData = $getAddressGroup->fetch_assoc();
                $member_lid = $getAddressGroupData['id'];
                ...
                if ($getMembers->num_rows > 0) {
                    while ($getMembersData = $getMembers->fetch_assoc()) {
                        ...
                        $original_address_member[] = $newMember.$newTable;
                    }
                }
            }
        }
        $getAddress = $projectdb->query("SELECT ipaddress,cidr,id,type FROM address WHERE BINARY name_ext='$translated_address' AND source='$source' AND vsys='$vsys' AND vtype='object';");
        if ($getAddress->num_rows == 1) {
            // $translated_address is a single address different that the $translated_address
            return false;
        } else {
            // get the members for the given $translated_address
            $getAddressGroup = $projectdb->query("SELECT id FROM address_groups_id WHERE BINARY name_ext='$translated_address' AND source='$source' AND vsys='$vsys';");
            if ($getAddressGroup->num_rows == 1) {
                $getAddressGroupData = $getAddressGroup->fetch_assoc();
                $member_lid = $getAddressGroupData['id'];
                $table_name = "address_groups_id";
                ...
                if ($getMembers->num_rows > 0) {
                    while ($getMembersData = $getMembers->fetch_assoc()) {
                        $newMember = $getMembersData['member_lid'];
                        $newTable = $getMembersData['table_name'];
                        $translated_address_member[] = $newMember.$newTable;
                    }
                }
            }
        }
        if(count($translated_address_member)===0 && count($original_address_member)===0){
            // given addresses has no members
            return false;
        } elseif (count(array_diff($original_address_member,$translated_address_member))===0){
            return true;
        }
        return false;
    }
...
function get_twice_nats($cisco_config_file, $source, $vsys, $template,$isafter) {
    ...
    $tp_dat_port="";
    $isdat="";
    $is_same_destination = 0; // MT-2493.
    $nat_port_translation = 0; // MT-2493.
    $source_pre_nat = "";
    $source_after_nat = "";
    $from="";
    $need_inbound_nat_rule = false; // MT-2597
    ...
    foreach ($cisco_config_file as $line => $names_line) {
        ...
        if ( (preg_match("/^nat \(/", $names_line)) AND (
            ...
            if(isset($netObj[10]) && $netObj[10]=== "unidirectional")
            {
                $need_inbound_nat_rule = false;
            } elseif(count($netObj)>9 &&
                $netObj[1] == "(any,any)" &&
                $netObj[2] == "source" &&
                $netObj[3] == "static" &&
                $netObj[6] == "destination" &&
                $netObj[7] == "static"
            ){
                // MT-2597 nat(any,any) source static X Y destination static X' Y'
                // this nat in CISCO means a bidirectional NAT rule - we need to create the inbound rule
                $need_inbound_nat_rule = true;
            }
...
//                    echo "$names_line is ***NOT*** After AUTO\n";
            ...
            //Remove the third field if it is a numeric value or it is "after-object" or "after-auto"
            if (isset($netObj[2]) && (($netObj[2]=="after-object") ||
                    (is_numeric($netObj[2])) ||
                    ($netObj[2]=="after-auto"))){
                unset($netObj[2]);
                $netObj = array_values($netObj);
            }
            if (is_numeric($netObj[2])){
                unset($netObj[2]);
                $netObj = array_values($netObj);
            }
            ...
            //Getting the Source section of the NAT. When finished with the source part, the entries in the netObj array are removed.
            if (isset($netObj[2]) && $netObj[2]=="source" && $netObj[3]=="dynamic") {
                #get Real IP Address by name

                //Getting the pre-NAT source
                ...
                if ($ObjectNetworkName == "any") {}
                    ...
                    if ($getRealIP->num_rows == 1) {
                        ...
                        $AddSource[$nat_lid]="('$source','$vsys','$member_lid','$table_name','$nat_lid')";
                    }
                    else {
                        ...
                        if ($getRealIP->num_rows == 1) {
                            ...
                            add_log2('error', 'Reading NAT', 'Nat Rule ID [' . $nat_lid . '] containg unknown pre-NAT source '.$ObjectNetworkName, $source, 'Review NAT rule','rules',$nat_lid,'nat_rules');
                        }
                    }
                    if (($from=="any") OR ($from=="")){
                        #Calculate the Zones based on Real IP
                        $from = getAutoZone($ipMapping['ipv4'], $member_lid, $table_name, "0");
                        /*if ($table_name=="address_groups_id"){
                            ...
                            if ($getMem->num_rows==1){
                                ...
                                add_log2('error', 'Reading NAT', 'Nat Rule ID [' . $nat_lid . '] containg unknown post-NAT source '.$ObjectNetworkName, $source, 'Review NAT rule','rules',$nat_lid,'nat_rules');
                            }
                        }
                        else{
                ...
                //Getting the post-NAT source
                $mappedIP = $netObj[5];
                if (compare_nat_address($mappedIP, $ObjectNetworkName, $source, $vsys)) {
                    # No Source NAT
                    $tp_sat_type="";
                }
                else {
                    ...
                    elseif ($mappedIP == "interface") {
                        ...
                        if ($getMappedIP->num_rows == 1) {
                            ...
                            $tp_sat_interface="";
                            $tp_sat_ipaddress="";
                            $AddTranslated[$nat_lid]="('$source','$vsys','$dst_lid','$dst_table','$nat_lid')";
                        }
                        else {
                            ...
                            if ($getMappedIP->num_rows == 1) {
                                ...
                                add_log2('error', 'Reading NAT', 'Nat Rule ID [' . $nat_lid . '] containg unknown post-NAT source '.$mappedIP, $source, 'Review NAT rule','rules',$nat_lid,'nat_rules');
                                echo "problem 7262\n";
                            }
                        }
                    }
                ...
                if ($ObjectNetworkName == "any") {}
                    ...
                    if ($getRealIP->num_rows == 1) {
                        ...
                        if ($getRealIP->num_rows == 1) {
                            ...
                            $query = "INSERT INTO address (name, name_ext, used, checkit, devicegroup, vsys, type, ipaddress, cidr, description, fqdn, v4, v6, vtype, source, invalid)    
                                                VALUES ('$ObjectNetworkName','$ObjectNetworkName','1','1','MT','$vsys','ip-netmask', '1.1.1.1','32','Missing object but used in a NAT','','1','0','object','$source','1')";
                            $projectdb->query($query);
                            $member_lid = $projectdb->insert_id;
                            $table_name = "address";
                            $AddSource[$nat_lid]="('$source','$vsys','$member_lid','$table_name','$nat_lid')";
                            add_log2('3', 'Reading NAT', 'Nat Rule ID [' . $nat_lid . '] containg missing  source "'.$ObjectNetworkName.'"', $source, 'Review NAT rule, or revise config and reimport','rules',$nat_lid,'nat_rules');
                        }
                    }
                    $source_pre_nat = $table_name;
                    if (($from=="any") OR ($from=="")){
                        #Calculate the Zones based on Real IP
                        $from = getAutoZone($ipMapping['ipv4'], $member_lid, $table_name, "0");
                        /*
                    ...
                    elseif ($mappedIP == "interface") {
                        ...
                        if ($getInterface->num_rows==1){
                            ...
                            if ($getObj->num_rows==1){
                                ...
                                $AddTranslated[$nat_lid]="('$source','$vsys','$int_member_lid','$int_table_name','$nat_lid')";
                            }
                            else{
                        ...
                        if ($getMappedIP->num_rows == 1) {
                            ...
                            if ($getMappedIP->num_rows == 1) {
                                ...
                                $query = "INSERT INTO address (name, name_ext, used, checkit, devicegroup, vsys, type, ipaddress, cidr, description, fqdn, v4, v6, vtype, source, invalid)    
                                                VALUES ('$mappedIP','$mappedIP','1','1','MT','$vsys','ip-netmask', '1.1.1.1','32','Missing object but used in a NAT','','1','0','object','$source','1')";
                                $projectdb->query($query);
                                $member_lid = $projectdb->insert_id;
                                $dst_table = "address";
                                $AddTranslated[$nat_lid]="('$source','$vsys','$member_lid','$dst_table','$nat_lid')";
                                add_log2('3', 'Reading NAT', 'Nat Rule ID [' . $nat_lid . '] containg missing  destination "'.$ObjectNetworkName.'"', $source, 'Review NAT rule, or revise config and reimport','rules',$nat_lid,'nat_rules');

                                echo "problem 7389\n";
                            }
                        }
                        $source_after_nat = $dst_table;
                        // MT-2500 do some fix on $tp_sat_type
                        if(($tp_sat_type === "static-ip") &&
                            ($source_pre_nat === "address_groups_id" && $source_after_nat === "address")
                        ) {
                            // MT-2500 - sat_type should be dynamic
                            $tp_sat_type="dynamic-ip-and-port";
                            $bidirectional = 0;
                        }
                    }
            ...
            //Getting the Destination section of the NAT. When finished with the desitnation part, the entries in the netObj array are removed.
            if (isset($netObj[2]) && ($netObj[2]=="destination") AND ($netObj[3]=="static")){
                ...
                #get Real IP Address by name
                //Getting the pre-NAT destination
                $ObjectNetworkName = $netObj[4];
                if ($ObjectNetworkName == "any") {}
                else {
                    $getRealIP = $projectdb->query("SELECT ipaddress,cidr,id,type FROM address WHERE BINARY name_ext='$ObjectNetworkName' AND source='$source' AND vsys='$vsys' AND vtype='object';");
                    if ($getRealIP->num_rows == 1) {
                        ...
                        $table_name = "address";
                        $RealIPType = $getRealIPData['type'];
                        $AddDestination[$nat_lid]="('$source','$vsys','$member_lid','$table_name','$nat_lid')";
                    }
                    else {
                        ...
                        if ($getRealIP->num_rows == 1) {
                            ...
                            $table_name = "address_groups_id";
                            $AddDestination[$nat_lid]="('$source','$vsys','$member_lid','$table_name','$nat_lid')";
                            // MT-2454 One limitation on Palo Alto is that when we do Destination NAT of type static,
                            // we cannot select an object of type “address-group”. We can only select an object of type “address”.
                            // so if destination is an address-group we need to change NAT to dynamic
                            $isdat="2";
                        }
                    }
                    #Calculate the Zones based on Real IP
                ...
                //Getting the post-NAT destination
                $mappedIP = $netObj[5];
                if ($mappedIP==$ObjectNetworkName) {
                    # No Destination NAT
                    $isdat=1;
                    $is_same_destination = 1; // MT-2493. We set as no Dnat due to destinations are the same, but we are still pending to check if there's some Destination port translation
                    $tp_dat_address_lid = $member_lid;
                    $tp_dat_address_table = $table_name;
                }
                else {
                    ...
                    elseif ($mappedIP == "interface") {
                        ...
                        if ($getMappedIP->num_rows == 1) {
                            ...
                            $getMappedIP = $projectdb->query("SELECT id FROM address_groups_id WHERE name_ext='$mappedIP' AND source='$source' AND vsys='$vsys';");
                            if ($getMappedIP->num_rows == 1) {
                                $getMappedIPData = $getMappedIP->fetch_assoc();
                                $tp_dat_address_lid=$getMappedIPData['id'];
                                $tp_dat_address_table = "address_groups_id";
                            } else{
                                add_log2('3', 'Reading NAT', 'Nat Rule ID [' . $nat_lid . '] containg unknown post-NAT destination '.$mappedIP, $source, 'Review NAT rule','rules',$nat_lid,'nat_rules');
                            }
                        }
                    }
                }
            ...
            if (isset($netObj[2]) && $netObj[2] == "service") {
                $nat_port_translation = 1; // MT-2493. As default value we set a destination Port translation is required
                #Destination Port Real
                $real_port = $netObj[3];
                $getdstPort = $projectdb->query("SELECT id, dport FROM services WHERE BINARY name_ext='$real_port' AND source='$source' AND vsys='$vsys';");
                ...
                $dst_port = $netObj[4];
                if ($dst_port==$real_port && $mappedIP==$ObjectNetworkName){
                    # No translate Port
                    //$tp_dat_port="";
                    $isdat=0; // we set as no DNAT due to destinations are the same and also there isn't NAT port translation
                } elseif ($dst_port!=$real_port && $mappedIP==$ObjectNetworkName){
                    $isdat=1; // MT-2493. despite destinations are the same we have NAT port translation so DNAT is enabled
                }
                else{
            ...
            if (($from=="any") AND ($op_zone_to!="")) {
                $from=$op_zone_to;
                //$AddFrom[$nat_lid]="('$vsys','$source','$from','$nat_lid')";
            }
            else{
                if(is_array($from)){
                    foreach($from as $from_item){
                        $AddFrom[$nat_lid] = "('$vsys','$source','$from_item','$nat_lid')";
                    }
                }else {
                    $AddFrom[$nat_lid] = "('$vsys','$source','$from','$nat_lid')";
                }
            ...
            // MT-2493. If we have same destinations but not port translation then DNAT should be disabled as destinations are the same
            if( $isdat!=="" && $isdat!==0 && $is_same_destination===1 && $nat_port_translation===0){
                $isdat = 0;
            }
            // MT-2493. If DNAT is disabled translated destination should be none
            if($isdat === 0){
                $tp_dat_address_lid = "";
                $tp_dat_address_table = "";
            }
            // MT-2493. We can not have a source bidireccional SNAT and DNAT at the same time.
            if($isdat!=="" && $isdat!==0 && $bidirectional==1){
                $bidirectional = 0;
                add_log2('warning', 'Reading Objects Nat', 'Nat RuleID [' . $nat_lid . '] has bidirectional source NAT and destination NAT but bidirectional has been disabled', $source, 'Check it manually', 'rules', $nat_lid, 'nat_rules');
            }

            $addNatRule[$nat_lid]="('$bidirectional','".normalizeComments($description)."','$source','$vsys','$nat_lid','$position','$disabled','$op_zone_to','$ruleName','$checkit','$tp_sat_address_type','$tp_sat_interface','$tp_sat_ipaddress','$tp_sat_type','$isdat','$tp_dat_address_lid','$tp_dat_address_table','$tp_dat_port','$op_service_lid','$op_service_table')";

            // MT-2597 nat(any,any) source static X Y destination static X' Y'
            // if the nat rule is something like nat(any,any) source static src_pre-nat src-post-nat destination static dst_pre-nat dst_post-nat then
            // Expedition needs to create the outbound and inbound nat rule
            if($need_inbound_nat_rule){

                if(isset($AddSource[$nat_lid])) {

                    $nat_new_lid = $nat_lid+1;

                    $outbound_source = str_replace(array("(", ")", "'"), "", $AddSource[$nat_lid]);
                    $outbound_source_array = explode(",", $outbound_source);
                    $inbound_tp_dat_address_lid = $outbound_source_array[2];
                    $inbound_tp_dat_address_table = $outbound_source_array[3];

                    // Add in $AddSource the $tp_dat_address_lid / $tp_dat_address_table
                    $AddSource[$nat_new_lid]="('$source','$vsys','$tp_dat_address_lid','$tp_dat_address_table','$nat_new_lid')";

                    if(isset($AddDestination[$nat_lid])) {
                        $outbound_destination = str_replace(array("(", ")", "'"), "", $AddDestination[$nat_lid]);
                        $outbound_destination_array = explode(",", $outbound_destination);
                        $AddTranslated[$nat_new_lid]="('$source','$vsys','".$outbound_destination_array[2]."','".$outbound_destination_array[3]."','$nat_new_lid')";
                    }
                    if(isset($AddTranslated[$nat_lid])) {
                        $outbound_tp_source = str_replace(array("(", ")", "'"), "", $AddTranslated[$nat_lid]);
                        $outbound_tp_source_array = explode(",", $outbound_tp_source);
                        // Add in $AddDestination the $tp_sat_ipaddress
                        $AddDestination[$nat_new_lid]="('$source','$vsys','".$outbound_tp_source_array[2]."','".$outbound_tp_source_array[3]."','$nat_new_lid')";
                       // get the to zone
                        $op_zone_to = ""; // none by default
                        if($outbound_tp_source_array[3]=="address_groups_id"){
                            $getMem=$projectdb->query("SELECT member_lid,table_name FROM address_groups WHERE lid='$outbound_tp_source_array[2]' AND table_name !='address_groups_id' LIMIT 1;");
                            if ($getMem->num_rows==1){
                                $getMemData=$getMem->fetch_assoc();
                                $mem_member_lid=$getMemData['member_lid'];
                                $mem_table_name=$getMemData['table_name'];
                            ...
                            $op_zone_to = search_zone_address_one($outbound_tp_source_array[2], $vsys, $source, $outbound_tp_source_array[3]);
                        }
                    }

                    if($tp_dat_address_table=="address"){
                        $from = search_zone_address_one($tp_dat_address_lid, $vsys, $source, $tp_dat_address_table);
                        $AddFrom[$nat_new_lid]="('$vsys','$source','$from','$nat_new_lid')";
                    } else if(count($ipMapping)>0){
                        $zones_from = getAutoZone($ipMapping['ipv4'], $member_lid, $table_name, "0");
                        foreach ($zones_from as $from_item){
                            $AddFrom[$nat_new_lid]="('$vsys','$source','$from_item','$nat_new_lid')";
                        }
                    }

                    $inbound_tp_sat_ipaddress = "";
                    $tp_sat_address_type = "translated-address";
                    $tp_sat_interface = "";
                    ...
                    $addNatRule[$nat_new_lid]="('$bidirectional','".normalizeComments($description)."','$source','$vsys','$nat_new_lid','$position','$disabled','$op_zone_to','${ruleName}-Inbound','$checkit','$tp_sat_address_type','$tp_sat_interface','$inbound_tp_sat_ipaddress','$tp_sat_type','$isdat','$inbound_tp_dat_address_lid','$inbound_tp_dat_address_table','$tp_dat_port','$op_service_lid','$op_service_table')";
...
//                if (isset($netObj[4]) AND ( $netObj[4] != "dns") AND ($netObj[4] != "route-lookup") AND $netObj[4] != "no-proxy-arp") {
                ...
                if (($netObj[2] == "static") AND ( isset($netObj[4]) AND $netObj[4] == "service")) {
                    ...
                    if (is_numeric($src_port)) {
                        ...
                        $cisco_service = get_service_by_protocol($src_port, $protocol, $source, $vsys);
                        if(isset($cisco_service['id'])){
                            $op_service_lid = $cisco_service['id'];
                    ...
                    if (!is_numeric($dst_port)) {
                        ...
                        $cisco_service = get_service_by_protocol($dst_port, $protocol, $source, $vsys);
                        if(isset($cisco_service['dport'])){
                            $tp_dat_port = $cisco_service['dport'];
...
//            }
            $addressGroupData = [
                ...
                'description'   => ''
                //'description'   => $description // description is not set ¿?
            ];
...
//                            }
                        ...
                        elseif($ipversion == "noip"){

                            add_log2('error', 'Reading Address Groups', 'Address group [' . $HostGroupName . '] has an invalid member object. ['. $names_line . ']', $source, 'Fix it manually', 'objects', $groupLid, 'address_groups_id');

                            // DO NOT ADD the address object into the database
...
function get_interfaces($cisco_config_file, $source, $vsys, $template) {
    ...
    foreach ($cisco_config_file as $line => $names_line) {
        ...
        if ($isInterface === TRUE) {
            ...
            }elseif (preg_match("/no nameif/i", $names_line)) {
